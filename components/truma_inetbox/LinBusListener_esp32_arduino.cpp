...
void LinBusListener::setup_framework() {
  ...
  uart_intr.intr_enable_mask =
      UART_RXFIFO_FULL_INT_ENA_M | UART_RXFIFO_TOUT_INT_ENA_M;  // nur RX FIFO FULL + TIMEOUT
  uart_intr.rxfifo_full_thresh = 1;       // Daten sofort verfügbar, nicht erst wenn FIFO voll
  uart_intr.rx_timeout_thresh = 10;       // kurze Nachrichten zuverlässig empfangen
  uart_intr.txfifo_empty_intr_thresh = 10;

  uart_intr_config(uart_num, &uart_intr); // Konfiguration anwenden
